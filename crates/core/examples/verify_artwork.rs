...
#[tokio::main]async fn main(){
 ...
 let outcome=tokio::spawn(async move{
  ...
  let started=Instant::now();let response=client.get(&url).query(&[("url",&image)]).bearer_auth(&first.token).send().await.unwrap();assert!(response.status().is_success(),"{}",response.status());let mime=response.headers()["content-type"].to_str().unwrap().to_owned();let bytes=response.bytes().await.unwrap();let cold_ms=started.elapsed().as_millis();assert!(!bytes.is_empty());assert!(bytes.len()<=galroon_core::artwork_cache::MAX_IMAGE_BYTES);
  ...
  json!({"separate_core":true,"live_provider_download":true,"restart_exact_bytes":true,"anonymous_denied":true,"bytes":bytes.len(),"mime":mime,"sha256":hex::encode(Sha256::digest(&bytes)),"cold_ms":cold_ms,"warm_ms":warm_ms,"cache_entries":1,"network_disabled":false,"source_files":0,"schema":galroon_core::db::SCHEMA_VERSION})
 }).await;
