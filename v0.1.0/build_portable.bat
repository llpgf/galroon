...
REM ===============================================
REM STEP 2.5: Run Database Migrations
REM ===============================================
echo.
echo [2.5/6] Running database migrations...
cd /d "%~dp0backend"
python -c "from app.alembic import command; command.upgrade('head')"
if %ERRORLEVEL% neq 0 (
    echo [WARNING] Failed to run migrations (may not be critical)
