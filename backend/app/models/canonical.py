...
class CanonicalGame:
    ...
    Attributes:
        ...
        is_curated: Sprint 9.5 - Whether this game is curated (shown in Gallery)
        overridden_fields: Sprint 10 - Per-field authority tracking (API vs Manual)
        created_at: Creation timestamp
        updated_at: Last update timestamp
    
    overridden_fields format:
        {"title": "manual", "description": "api", "cover_image_url": "manual"}
        - "manual" = user has explicitly set this field
        - "api" = field comes from external source (VNDB, etc.)
    """
    ...
    is_curated: bool = False  # Sprint 9.5: Gallery/Workshop dual view
    overridden_fields: Dict[str, str] = None  # Sprint 10: Per-field authority
    created_at: str = ""
    updated_at: str = ""
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CanonicalGame':
        ...
        # Handle missing is_curated for existing records
        if 'is_curated' not in data:
            data['is_curated'] = False
        # Handle missing overridden_fields for existing records
        if 'overridden_fields' not in data or data['overridden_fields'] is None:
            data['overridden_fields'] = {}
        elif isinstance(data['overridden_fields'], str):
            data['overridden_fields'] = json.loads(data['overridden_fields'])
        return cls(**data)
...
CANONICAL_GAMES_TABLE_SQL = """
    CREATE TABLE IF NOT EXISTS canonical_games (
        ...
        is_curated INTEGER DEFAULT 0,  -- Sprint 9.5: Gallery/Workshop dual view
        overridden_fields TEXT DEFAULT '{}',  -- Sprint 10: Per-field authority JSON
...
CANONICAL_INDEXES_SQL = [
    ...
    "CREATE INDEX IF NOT EXISTS idx_canonical_is_curated ON canonical_games(is_curated)",  # Sprint 9.5
